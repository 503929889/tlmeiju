...
<body>
			...
			<div class="panel panel-success">
				<div class="panel-heading">
					<h3 class="panel-title">
						<strong>${typeList.rows[0].type.typeName}</strong>
					</h3>
				</div>
				<!--	body	-->
				<div class="panel-body row" style="margin-top: -10px;" id="typePage">
					<c:forEach var="item" items="${typeList.rows}">
						<div class="col-lg-3 col-md-3 col-sm-4 col-xs-6 box-bg">
							<div class="box-sm">
								...
								<div class="box-text" style="font-size: 12px;">
									<div class="pull-left">
										<a href="detail_show?titleId=${item.titleId}"><strong>${item.titleName}</strong></a><br>
										<span style="color: #999;">状态：</span><span
											style="color: #FF2626">更新:${item.status}</span><br> <span
											style="color: #999;">年份：</span><span><fmt:formatDate
												value="${item.premiereDate}" type="both" pattern="yyyy" /></span>
									</div>
				...
				<!-- <a class="btn btn-info">1</a><a class="btn btn-info">2</a><a class="btn btn-info">3</a> -->
				<!--	body	-->
			</div>
		</div>
		<!--	right box  -->
	...
	<input type="hidden" id="typeId" name="typeId"
		value="${typeList.rows[0].type.typeId}" />
	...
	function loadMeiju(data){
		var t=data.rows;
		for(var i=0;i<t.length;i++){
			$box_bg=$('<div class="col-lg-3 col-md-3 col-sm-4 col-xs-6 box-bg"></div>');
			$box_sm=$('<div class="box-sm"></div>');
			$box_img=$('<div class="box-img"></div>');
			$a=$('<a href="showDetails?titleId='+t[i].titleId+'" title="'+t[i].titleName+'"></a>');
			$img=$('<img src="'+t[i].picture+'">');
			$a.append($img);
			$box_img.append($a);
		 	$box_text=$('<div class="box-text" style="font-size: 12px;"></div>');
			$div_l=$('<div class="pull-left"></div>');
			$a1=$('<a href="showDetails?titleId='+t[i].titleId+'"><strong>'+t[i].titleName+'</strong></a><br>');
			$span1=$('<span style="color: #999;">状态：</span>');
			$span2=$('<span style="color: #FF2626">更新:'+t[i].status+'</span><br>');
			var year=new Date(t[i].premiereDate).getFullYear();
			$span3=$('<span style="color: #999;">年份：'+year+'</span>');
			$div_l.append($a1);
			$div_l.append($span1);
			$div_l.append($span2);
			$div_l.append($span3);
			$div_r=$('<div class="pull-right"><a href="http://list.youku.com" class="btn btn-info btn-sm"> <span class="glyphicon glyphicon-play-circle"></span> Play </a></div>');	
			$box_text.append($div_l);
			$box_text.append($div_r);
			$box_sm.append($box_img);
			$box_sm.append($box_text);
			$box_bg.append($box_sm);
			$("#typePage").append($box_bg);
